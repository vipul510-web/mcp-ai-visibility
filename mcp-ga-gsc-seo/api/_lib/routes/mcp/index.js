...
const SERVER_INFO = {
    name: process.env.MCP_SERVER_DISPLAY_NAME || 'mcp-ga-gsc-seo',
    version: '0.1.0',
};
const INSTRUCTIONS = `You are connected to a user's Google Analytics 4 and Google Search Console data via this MCP server.
...
const PROMPTS = [
    {
        ...
        template: () => 'Using the GA4 and Search Console MCP tools, run find_ctr_opportunities for my Search Console site. For each of the top 10 results, give me a rewritten page title and meta description that better matches the query intent. Explain the reasoning in one short sentence each.',
    },
    {
        name: 'content_gaps',
...
<h2>GA4 + Search Console MCP endpoint</h2>
<p>This URL is the MCP server endpoint. Add it as a custom connector in Claude.ai (or another MCP client).</p>
