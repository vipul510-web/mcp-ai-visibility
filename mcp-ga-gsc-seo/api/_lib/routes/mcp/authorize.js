...
import { lookupClient, createAuthCode, normalizeMcpAudience } from '../../mcp-auth.js';

/** Rebuild authorize URL from params so post–Google OAuth return works even if `req.url` is a rewrite target. */
function buildMcpAuthorizeReturnPath(q) {
    ...
    if (!sp.get('response_type')) sp.set('response_type', 'code');
    if (!sp.get('code_challenge_method')) sp.set('code_challenge_method', 'S256');
    return `/api/mcp/authorize?${sp.toString()}`;
}

function clientRedirectUris(client) {
    let allowed = client.redirect_uris;
...
export default async function handler(req, res) {
    try {
        ...
        const resource = q.resource ? normalizeMcpAudience(String(q.resource)) : null;
        ...
        const allowed = clientRedirectUris(client);
        ...
        if (!session || !session.uid) {
            // Bounce through Google OAuth, preserving this URL (rebuilt — do not rely on `req.url` after Vercel rewrites).
            const returnTo = buildMcpAuthorizeReturnPath(q);
            res.setHeader('Set-Cookie', serializeCookie('sol_return_to', returnTo, {
                httpOnly: true,
        ...
        const authorizeReturnPath = buildMcpAuthorizeReturnPath(q);
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        return res.status(200).send(renderConsent({
            user,
            clientName,
            scope,
            continueUrl: `${authorizeReturnPath}&confirm=1`,
        }));
    } catch (err) {
...
<title>Authorise ${safe(clientName)} | MCP</title>
<style>
...
<div class="card">
    <div class="brand">
        <span style="font-weight:800;font-size:1.05rem">GA4 + GSC MCP</span>
    </div>
    ...
    <p>${safe(clientName)} is asking to use your connected Google Analytics and Search Console through this MCP server.</p>
    <ul>
