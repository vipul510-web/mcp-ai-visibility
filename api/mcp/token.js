...
async function handleRefresh(body, res) {
    ...
    const access = signAccessToken({ userId: payload.sub, clientId: client_id, resource: payload.aud, scope: 'mcp.read' });
    const refresh = signRefreshToken({ userId: payload.sub, clientId: client_id, resource: payload.aud });
